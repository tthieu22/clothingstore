...
class login
{
    ...
    {
        $today = date("Y-m-d");
        $query = "SELECT SUM(TongTien) as Count FROM HoaDon WHERE DATE(NgayLap) = '$today' And TrangThai = 1";
        return $this->conn->query($query)->fetch_assoc();
    }
    function tk_dtthang($m)
    {
        $y = date("Y"); // Lấy năm hiện tại
        $query = "SELECT SUM(TongTien) as Count FROM HoaDon WHERE MONTH(NgayLap) = $m AND YEAR(NgayLap) = $y AND TrangThai = 1";
        return $this->conn->query($query)->fetch_assoc();
    }
    function tk_dtnam($y)
    {
    ...
    {
        $query = "SELECT DAY(NgayLap) as Day, SUM(TongTien) as Revenue 
                  FROM HoaDon 
...
{
    $query = "SELECT MONTH(NgayLap) as Month, SUM(TongTien) as Revenue 
              FROM HoaDon 
    ...
    {
        $query = "SELECT YEAR(NgayLap) as Year, SUM(TongTien) as Revenue 
                  FROM HoaDon 
                  WHERE TrangThai = 1 
